...
static node_t *_rlu_new_node(int key)
{
	node_t *node = RLU_ALLOC_UNSAFE(sizeof(node_t), true);
...
void update_n_threads(int n_threads) 
{
...
void *list_global_init(int init_size, int value_range)
{
	...
	int i, key, val, direction;
		...
		conf.n_threads = g_n_threads;
		printf("########## n_thread= %d ############ \n", g_n_threads);
		printf("########## conf.n_thread= %d ############ \n", conf.n_threads);
		RLU_INIT(&conf);
#endif
	...
	if (tree == NULL)
		return NULL;
	tree->root = _rlu_new_node(INT_MAX);
