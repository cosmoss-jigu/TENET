...
node_t *_rlu_new_node(rlu_thread_data_t *self) {

#ifdef IS_MVRLU
	node_t *p_new_node = (node_t *)RLU_ALLOC_UNSAFE(sizeof(node_t), true);
	if (p_new_node == NULL){
		printf("out of memory\n");
		exit(1);
	}
...
list_t *rlu_new_list(rlu_thread_data_t *self)
{
	...
	p_max_node = _rlu_new_node(self);
	p_max_node->val = LIST_VAL_MAX;
	p_max_node->p_next = NULL;

	p_min_node = _rlu_new_node(self);
	p_min_node->val = LIST_VAL_MIN;
	p_min_node->p_next = p_max_node;
...
void rlu_list_print(rlu_thread_data_t *self, list_t *p_list) {
	...
	if (RLU_READER_UNLOCK(self) == ESPATIAL) {
		perror("spatial safety violation detected \n");
		exit(0);
	}
}
...
restart:
	...
	/* TODO: check return value for unlock
	 * spatial safety and temporal safety check*/
	//RLU_READER_UNLOCK(self);
	if (RLU_READER_UNLOCK(self) == ESPATIAL) {
		perror("spatial safety violation detected \n");
		exit(0);
