...
static ts_nvm_root_obj_t *__g_root_obj, *__g_root_obj_numa;

unsigned long nvm_get_gen_id(void) { return __g_root_obj->gen_id; }
...
ts_nvm_root_obj_t *nvm_init_numa_heap(const char *path, size_t sz,
                                      int *p_need_recovery) {
  ts_nvm_root_obj_t *root_obj;
  int is_created, need_recovery;

  /* Load a nvm heap from a file */
  root_obj = nvm_load_numa_heap(path, sz, &is_created);
  if (unlikely(!root_obj)) {
    return NULL;
  }
...
void nvm_free(void *ptr) { port_free(ptr); }
