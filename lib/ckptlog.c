...
static ts_ckpt_entry_t *_ckptlog_enq(ts_ckptlog_t *ckptlog,
				     ...
				     ts_cpy_hdr_struct_t *chs, bool is_replica)
{
	...
	ckpt_entry->nvlog_hdr.checksum = generate_obj_checksum(
		ckpt_entry->obj_hdr.obj, TYPE_NVLOG_ENTRY);
			     ...
			     bool is_replica)
{
	...
	while ((ckpt_entry = ckptlog_peek_head(ckptlog))) {
		...
		if (unlikely(ckptlog_hdr->tombstone)) {
			...
			if (!is_replica) {
				p_act_vhdr =
					get_vact_vhdr(ckptlog_hdr->np_org_act);
				ts_assert(p_act_vhdr->p_lock);
				if (!p_act_vhdr) {
					ts_trace(
						TS_INFO,
						"NULL VHDR being added to teh ckpt free_set\n");
					exit(0);
				}
	...
	while ((ckpt_entry = ckptlog_peek_head(ckptlog))) {
		...
		if (p_act_vhdr->np_cur_act == ckpt_entry->obj_hdr.obj &&
			...
			ckptlog->tx_replica_set[tail].src =
				vobj_to_ahs(p_act_vhdr->np_org_act);
			ckptlog->tx_replica_set[tail].len =
				sizeof(ts_act_hdr_struct_t) +
				ckpt_entry->obj_hdr.obj_size + CANARY_SIZE;
			ckptlog->tx_replica_set[tail].offset = get_pool_offset(
				vobj_to_ahs(p_act_vhdr->np_org_act));
			ckptlog->tx_replica_set[tail].commit_clk =
				ckptlog->clks->__qp0;
...
void ckptlog_reclaim(ts_ckptlog_t *ckptlog, ts_ckptlog_t *ckptlog_replica)
{
	...
	case RECLAIM_CKPTLOG_BEST_EFFORT:
		...
		ret = ckptlog_reclaim_x(ckptlog_replica, ckptlog->log_size + 1,
					true);
#endif
		if (likely(ret)) {
...
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
		/* TODO: reclaim the replica*/
		ret = ckptlog_reclaim_x(ckptlog_replica, TS_CKPTLOG_LOW_MARK,
					true);
#endif
		if (likely(ret)) {
