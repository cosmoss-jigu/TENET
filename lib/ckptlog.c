...
int ckptlog_create(ts_thread_struct_t *self, ts_ckptlog_t *ckptlog,
  ...
  rc = nvlog_create(self, ckptlog, size, TYPE_CKPTLOG, status, is_replica);
  if (likely(!rc)) {
    ckptlog->free_set = &self->ckpt_free_set;
    ckptlog->need_reclaim = &self->reclaim.ckptlog;
#ifdef TS_ENABLE_MOBJ_REPLICATION
    ckptlog->tx_replica_set = self->tx_replica_set;
    ckptlog->rs_head = &self->rs_head;
    ckptlog->rs_tail = &self->rs_tail;
#endif
  }
...
void ckptlog_destroy(ts_ckptlog_t *ckptlog, ts_ckptlog_t *ckptlog_replica) {
  ...
  nvlog_destroy(ckptlog);

#ifdef TS_ENABLE_CKPTLOG_REPLICATION
  nvlog_destroy(ckptlog_replica);
#endif
  return;
}
...
static ts_ckpt_entry_t *_ckptlog_enq(ts_ckptlog_t *ckptlog,
  ...
  ckpt_entry = (ts_ckpt_entry_t *)nvlog_enq(
      ckptlog, (sizeof(ts_ckpt_entry_hdr_t) + sizeof(ts_obj_hdr_t) + obj_size));
#ifdef TS_GTEST
  ...
  ckpt_entry->ckptlog_hdr.np_org_act = chs->cpy_hdr.p_act_vhdr->np_org_act;
  ...
  ckpt_entry->nvlog_hdr.checksum =
      generate_obj_checksum(ckpt_entry->obj_hdr.obj, TYPE_NVLOG_ENTRY);

  /* update np_cur_master to the new master */
  /* TODO: Optimize for the replica*/
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
  if (!is_replica) {
...
ts_ckpt_entry_t *ckptlog_enq(ts_ckptlog_t *ckptlog, unsigned long wrt_clk,
  ...
  return _ckptlog_enq(ckptlog, wrt_clk, ckpt_s_clk, chs, is_replica);
}
...
void ckptlog_deq_persist(ts_ckptlog_t *ckptlog) { nvlog_deq_persist(ckptlog); }
...
void ckptlog_cleanup(ts_ckptlog_t *ckptlog, unsigned long until_clk) {
  ...
  ts_ptr_set_t *free_set = ckptlog->free_set;
  ...
  for (i = 0, first = 0, nfree = 0; i < free_set->num_ptrs; ++i) {
    ...
    if (lt_clock(tombstone_clk, until_clk)) { /* TIME: < */
      ...
      ts_trace(TS_NVMFREE, "[%s:%d] NVM_FREE (ORG_ACT): %p (%lx <= %lx) \n",
               __func__, __LINE__, p_act_vhdr->np_org_act, tombstone_clk,
               until_clk);
      nvm_free(ahs);
...
static int ckptlog_reclaim_x(ts_ckptlog_t *ckptlog, unsigned long watermark,
   ...
   * one grace period once they were written back to the
  ...
  while ((ckpt_entry = ckptlog_peek_head(ckptlog))) {
    ...
    if (unlikely(ckptlog_hdr->tombstone)) {
       ...
       */
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
      /* collect the free object only for the main ckptlog
       * collecting for replica results in duplicate copy in ptr_set
       * so simply skip this step if replica is being reclaimed*/
      if (!is_replica) {
        p_act_vhdr = get_vact_vhdr(ckptlog_hdr->np_org_act);
        ts_assert(p_act_vhdr->p_lock);
        if (!p_act_vhdr) {
          ts_trace(TS_INFO, "NULL VHDR being added to teh ckpt free_set\n");
          exit(0);
        }
  ...
  while ((ckpt_entry = ckptlog_peek_head(ckptlog))) {
    ...
    /* Is it time to stop? */
    if (gt_clock(ckptlog_hdr->ckpt_s_clk, wb_limit_clk)) { /* TIME: > */
      break;
    }
    ...
    /* np_cur_act will be different for replica if main
     * ckptlog reclaim has performed the writeback
     * for replica the cpy obj can be directly reclaimed
     * Replica will stop at the same point as of the main
     * ckptlog*/
...
#ifdef TS_ENABLE_MOBJ_REPLICATION
      /* prepare the updated nvobj for replication to the disk */
      tail = (*ckptlog->rs_tail + 1) % MAX_SET_SIZE;
      if (tail == *ckptlog->rs_head ||
          (*ckptlog->rs_head == -1 && tail == MAX_SET_SIZE - 1)) {
        perror("Ckpt: Replica set size exceeded \n");
        exit(0);
      }
      ckptlog->tx_replica_set[tail].src = vobj_to_ahs(p_act_vhdr->np_org_act);
      ckptlog->tx_replica_set[tail].len = sizeof(ts_act_hdr_struct_t) +
                                          ckpt_entry->obj_hdr.obj_size +
                                          CANARY_SIZE;
      ckptlog->tx_replica_set[tail].offset =
          get_pool_offset(vobj_to_ahs(p_act_vhdr->np_org_act));
      ckptlog->tx_replica_set[tail].commit_clk = ckptlog->clks->__qp0;
      *ckptlog->rs_tail = tail;
#endif

      /* ... and update np_cur_act. */
      smp_atomic_store(&p_act_vhdr->np_cur_act, p_act_vhdr->np_org_act);
  ...
  if (new_head_cnt != old_head_cnt) {
    ...
    ts_trace(TS_CKPTLOG,
             ...
             req2str(*ckptlog->need_reclaim), ckptlog, reclaimed_bytes,
             ckptlog->head_cnt, ckptlog->prev_head_cnt, ckptlog->tail_cnt);
  }
  stat_thread_acc(ckptlog->thread, n_ckptlog_writeback_bytes, writeback_bytes);

  /* Unlock */
  smp_wmb_tso();

#ifdef TS_ENABLE_CKPTLOG_REPLICATION
  /* turn off need_reclaim only after the successfully reclaiming the
   * replica as well*/
  if (is_replica)
...
void ckptlog_reclaim(ts_ckptlog_t *ckptlog, ts_ckptlog_t *ckptlog_replica) {
  int ret;

  switch (*ckptlog->need_reclaim) {
  case RECLAIM_CKPTLOG_BEST_EFFORT:
    ret = ckptlog_reclaim_x(ckptlog, ckptlog->log_size + 1, false);
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
    /* TODO: reclaim the replica*/
    ret = ckptlog_reclaim_x(ckptlog_replica, ckptlog->log_size + 1, true);
#endif
    if (likely(ret)) {
  ...
  case RECLAIM_CKPTLOG_WRITEBACK:
    ret = ckptlog_reclaim_x(ckptlog, TS_CKPTLOG_LOW_MARK, false);
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
    /* TODO: reclaim the replica*/
    ret = ckptlog_reclaim_x(ckptlog_replica, TS_CKPTLOG_LOW_MARK, true);
#endif
    if (likely(ret)) {
  ...
  case RECLAIM_CKPTLOG_WRITEBACK_ALL:
    ret = ckptlog_reclaim_x(ckptlog, 0, false);
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
    /* TODO: reclaim the replica*/
    ret = ckptlog_reclaim_x(ckptlog_replica, 0, true);
#endif
    if (likely(ret)) {
      stat_thread_inc(ckptlog->thread, n_ckptlog_writeback);
    }
...
void ckptlog_flush(ts_ckptlog_t *ckptlog, ts_ckptlog_t *ckptlog_replica) {
  if (*ckptlog->need_reclaim) {
