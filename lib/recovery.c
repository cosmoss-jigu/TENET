...
static int reconstruct_volatile_header(ts_ckpt_entry_t *ckpt_entry,
  ...
  if (act_nvhdr.gen_id != gen_id || !act_nvhdr.p_act_vhdr) {
    p_act_vhdr = (ts_act_vhdr_t *)alloc_act_vhdr(ckpt_entry->obj_hdr.obj);
    ...
    cur_act_ckpt_entry =
        (ts_ckpt_entry_t *)obj_to_ckpt_ent((void *)p_act_vhdr->np_cur_act);
    if (cur_act_ckpt_entry->nvlog_hdr.wrt_clk < ckpt_entry->nvlog_hdr.wrt_clk) {
...
int ckptlog_recovery(ts_recovery_t *recovery) {
  ...
  for (; current; current = current->next) {
    ...
    while (nvlog->head_cnt != nvlog->tail_cnt) {
      ...
      if (ckpt_entry->ckptlog_hdr.ckpt_s_clk > last_ckpt_clk) {
        break;
      }
...
static int alloc_replay_array(ts_nvm_root_obj_t *root,
  ...
  for (ops_cnt = 0; current; current = current->next) {
    if (current->type != TYPE_OPLOG || current->status != STATUS_NVLOG_NORMAL) {
      continue;
    }
...
static int collect_replay_entries(ts_nvm_root_obj_t *root,
  ...
  for (i = 0; current; current = current->next) {
    ...
    if (unlikely(!nvlog)) {
      perror(
          "failed to construct volatile nvlog for oplog in collect entries \n");
      return ENOMEM;
    }
    head_cnt = nvlog->head_cnt;
    while (nvlog->head_cnt != nvlog->tail_cnt) {
...
static void init_threads(ts_ptr_set_t *threads) { ptrset_init(threads); }
...
static int execute_op(ts_op_exec_fn_t op_exec, ts_replay_entry_t *re_ent) {
  ...
  case OPLOG_REPLAY_TYPE_EXEC:
     ...
     * at the user side*/
    op_exec(context->thread, re_ent->type, context->op_entry->oplog_hdr.opd);
    break;
...
int oplog_recovery(ts_recovery_t *recovery) {
  ...
  ret = collect_replay_entries(root, replay_array, context_array, &ops_cnt);
  if (unlikely(ret)) {
