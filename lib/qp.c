...
#define thread_list_for_each_safe(tl, pos, n, thread)                          \
  for (pos = (tl)->list.next, n = (pos)->next, thread = list_to_thread(pos);   \
       pos != &(tl)->list;                                                     \
 ...
 */
static inline void advance_qp_clock(ts_sys_clks_t *clks,
...
#ifdef TS_ENABLE_MOBJ_REPLICATION
bool *get_drain_ptr(void) { return (void *)&g_qp_thread.wait_for_drain; }
#endif
...
static void qp_init(ts_qp_thread_t *qp_thread, unsigned long qp_clk) {
  ...
  thread_list_lock(&g_live_threads);
  {
    ...
    thread_list_for_each_safe(&g_live_threads, pos, n, thread) {
      ...
      thread->qp_info.need_wait = thread->qp_info.run_cnt & 0x1;
    }
  }
  ...
  thread_list_lock(&g_live_threads);
  {
    thread_list_for_each_safe(&g_live_threads, pos, n, thread) {
      ...
      while (1) {
        ...
        if (thread->qp_info.run_cnt != thread->run_cnt ||
...
static bool init_work_data(ts_qp_thread_t *qp_thread) {
  ...
  if (th_cnt == g_n_threads) {
    ts_trace(TS_DEBUG, " !!!!! IO worker thread data initialized !!!!!!! \n");
  }
...
static void prep_and_submit_io_reqs(io_work *wrk) {
  ...
  for (int i = 0; i < g_th_per_worker; ++i) {
    ...
    retry:
      ...
      ts_trace(TS_DEBUG, "n_off= %ld\t d_off= %ld\t f_id= %d\t f_ind= %d\n",
               entry.offset, disk_offset, f_id, fd_index);

      /* using global list to keep track of written fds*/
      if ((need_flush = smp_atomic_load(&g_fd_flush_list[fd_index])) == false)
        smp_atomic_store(&g_fd_flush_list[fd_index], true);

      io_uring_prep_write(sqe, fd_index, entry.src, entry.len, disk_offset);
...
static void create_io_workers(ts_qp_thread_t *qp_thread, unsigned long qp_clk) {
  ...
  for (int i = 0; i < g_n_workers; ++i) {
    ...
    ret = pthread_create(&qp_thread->workers[i], NULL, io_uring_main,
...
static bool write_to_disk_async(ts_qp_thread_t *qp_thread,
  ...
  thread_list_lock(&g_live_threads);
  {
    ...
    for (int i = 0; i < g_n_workers; ++i) {
      ...
      if (ret != 0) {
        ts_trace(TS_INFO, "Error waiting for IO worker %d \n", i);
      }
  ...
  if (n_reqs_sub != n_reqs_comp) {
    ts_trace(TS_INFO, "IO error: n_reqs_sub= %d \t n_reqs_comp= %d\n",
    ...
    thread_list_for_each_safe(&g_live_threads, pos, n, thread) {
      ...
      if (unlikely(thread->reclaim.requested)) {
        ...
        if (thread->reclaim.ckptlog)
          ckptlog_reclaim(&thread->ckptlog, &thread->ckptlog_replica);
      }
    }
...
static int wait_for_disk_writes(void *arg) {
  ...
  for (int i = 0; i < g_n_io_uring_inst; ++i) {
    ...
    if (!qp_thread->n_disk_entries[i])
      continue;
    for (unsigned int j = 0; j < qp_thread->n_disk_entries[i]; ++j) {
...
static int prep_io_uring_reqs(ts_qp_thread_t *qp_thread,
  ...
  retry:
    ...
    if (!sqe) {
      ...
      if (ret != n_current) {
        ts_trace(TS_INFO, "n_entries_polled= %d \t  n_entries_submitted= %d\n",
                 ret, n_current);
  ...
  thread_list_lock(&g_zombie_threads);
  {
    ...
    thread_list_for_each_safe(&g_zombie_threads, pos, n, thread) {
      ...
      if (tvlog_used(&thread->tvlog)) {
        *thread->tvlog.need_reclaim = RECLAIM_TVLOG_CKPT;
        tvlog_reclaim(&thread->tvlog, &thread->ckptlog,
                      &thread->ckptlog_replica);
      }

      if (ckptlog_used(&thread->ckptlog)) {
        *thread->ckptlog.need_reclaim = RECLAIM_CKPTLOG_WRITEBACK_ALL;
        ckptlog_reclaim(&thread->ckptlog, &thread->ckptlog_replica);
      }
      ...
      if (tvlog_used(&thread->tvlog) == 0 &&
        ...
        ckptlog_destroy(&thread->ckptlog, &thread->ckptlog_replica);
        oplog_destroy(&thread->oplog, &thread->oplog_replica);

#ifdef TS_ENABLE_MOBJ_REPLICATION
        /* submit all the pending writes in the replica set for disk
         * replication before freeing the thread*/
        cnt = prep_io_uring_reqs(qp_thread, thread, qp_thread->clks.__qp0,
                                 is_drain);
        if (cnt) {
        ...
        if (thread->live_status == THREAD_DEAD_ZOMBIE) {
          thread_list_del_unsafe(&g_zombie_threads, thread);
...
retry:
  ...
  thread_list_lock(&g_live_threads);
  {
    ...
    thread_list_for_each_safe(&g_live_threads, pos, n, thread) {
      ...
      if (thread->clks.__min_ckpt_reclaimed < min_ckpt_reclaimed) {
        min_ckpt_reclaimed = thread->clks.__min_ckpt_reclaimed;
      }
    }
...
static void qp_trigger_reclaim(ts_qp_thread_t *qp_thread,
  ...
  thread_list_lock(&g_live_threads);
  {
    ts_trace(TS_QP,
             ...
             qp_thread->clks.__min_ckpt_reclaimed, req2str(reclaim.tvlog),
             req2str(reclaim.ckptlog));
...
static inline int should_stop(ts_qp_thread_t *qp_thread) {
  return qp_thread->stop_requested && thread_list_empty(&g_zombie_threads) &&
...
static void __qp_thread_main(void *arg) {
  ...
  while (!should_stop(qp_thread)) {
    ...
    /* Print out message */
    ts_trace(TS_QP, "[QP:%ld/%ld/%ld] ----- Reclamation barrier passed -----\n",
             qp_thread->clks.__qp0, qp_thread->clks.__last_ckpt,
...
static int init_qp_thread(ts_qp_thread_t *qp_thread) {
  ...
  /* init wrk data*/
  if (!(qp_thread->workers =
            (pthread_t *)port_alloc(sizeof(pthread_t) * g_n_workers))) {
  ...
  for (int i = 0; i < g_n_workers; ++i) {
    ...
    qp_thread->n_disk_entries[i] = 0;
    qp_thread->work_data[i].n_disk_entries = &qp_thread->n_disk_entries[i];
    qp_thread->work_data[i].n_file_writes = qp_thread->n_file_writes;
    if (!(qp_thread->work_data[i].thread = (ts_thread_struct_t **)port_alloc(
              sizeof(ts_thread_struct_t *) * g_th_per_worker))) {
      ts_trace(TS_INFO, "Error allocating workers data\n");
      exit(0);
    }
  ...
  rc = port_create_thread("qp_thread", &qp_thread->thread, &qp_thread_main,
                          qp_thread, &qp_thread->completion);
  if (rc) {
...
static void update_thread_count(int n_threads) {
  ...
  if (n_threads % N_WORKERS_MAX) {
    ts_trace(TS_INFO, "invalid thread count, adjust the MAX workers\n");
    exit(0);
  }
  g_n_threads = n_threads;
}
