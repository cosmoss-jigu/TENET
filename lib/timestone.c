...
static inline unsigned long get_gen_id(void) { return __g_gen_id; }
...
int __init ts_init(ts_conf_t *conf) {
  ...
  nvm_root_obj = nvm_init_heap(conf->nvheap_path,
  ...
  nvm_root_obj_numa = nvm_init_numa_heap(
      conf->nvheap_path, align_size_t_to_pmem_page(conf->nvheap_size),
      &need_recovery);
...
void __ts_unload_nvm(void) { nvm_heap_destroy(); }
...
void ts_thread_free(ts_thread_struct_t *self) {
  ...
  smp_mb();
#ifdef TS_ENABLE_MOBJ_REPLICATION
  if (tvlog_used(&self->tvlog) == 0 && ckptlog_used(&self->ckptlog) == 0 &&
...
void ts_thread_init_x(ts_thread_struct_t *self, unsigned short flags) {
  ...
  ckptlog_create(self, &self->ckptlog_replica, TS_CKPTLOG_SIZE, flags, true);
...
static inline void try_reclaim_logs(ts_thread_struct_t *self) {
  if (unlikely(self->reclaim.requested)) {
    if (self->reclaim.tvlog)
      tvlog_reclaim(&self->tvlog, &self->ckptlog, &self->ckptlog_replica);
...
void tss_insert_canary_act(ts_act_hdr_struct_t *ahs) {
  ...
  *p_obj_canary = ahs->obj_hdr.canary ^ obj_canary_mask;
  ts_trace(TS_DEBUG, "C0= %ld \t C1= %ld \t C2= %ld\n", ahs->act_nvhdr.canary,
           ahs->obj_hdr.canary, *p_obj_canary);
  ts_trace(TS_DEBUG, "=========================================\n");
}
...
static void update_nv_orig_obj(ts_thread_struct_t *self) {
  ...
  for (i = 0; i < num_v_ptrs; ++i) {
    ...
    ts_assert(obj_to_ahs(nvobj)->act_nvhdr.checksum == INVALID_CHECKSUM);
    obj_to_ahs(nvobj)->act_nvhdr.checksum =
        generate_obj_checksum(nvobj, obj_to_obj_hdr(nvobj)->type);
...
void replicate_mobj_to_disk(ts_thread_struct_t *self,
                            ts_disk_entry_t *entries) {
#if 0	
	int i;
...
int ts_end(ts_thread_struct_t *self) {
  ...
  if (!allocate_nv_orig_obj(self)) {
    ts_trace(TS_INFO, "Aborting the Transaction for graceful exit\n");
    return ESPATIAL;
  }
  ...
  if (self->is_write_detected) {
    ret = tvlog_commit(&self->tvlog, &self->oplog, &self->oplog_replica,
                       &self->tx_free_set, self->local_clk, &self->op_info);
    if (!ret) {
      ts_trace(TS_INFO, "Aborting the Transaction for graceful exit\n");
      return ESPATIAL;
    }
...
void ts_abort(ts_thread_struct_t *self) {
  ...
  free_new_vact(self);
  if (self->tx_alloc_set.num_ptrs != 0 || self->tx_assign_set.num_ptrs != 0) {
    ts_trace(TS_INFO, "num_alloc_ptrs= %d \t num_assign_ptrs= %d",
             self->tx_alloc_set.num_ptrs, self->tx_assign_set.num_ptrs);
    ts_assert(self->tx_alloc_set.num_ptrs == 0);
    ts_assert(self->tx_assign_set.num_ptrs == 0);
  }
...
void _dbg_assert_chs_copy(const char *f, const int l,
...
void *ts_deref(ts_thread_struct_t *self, void *obj) {
  ...
  if (p_act_vhdr) {
    ...
    if (is_ptr_valid(p_act_vhdr, self->local_clk) == ETEMPORAL) {
      perror(
          "Temporal safety (user-after-free) violation detected (cpy_obj)\n");
      // FIXME: return ETEMP
      exit(0);
    }
  }
  ...
  if (unlikely(p_copy)) {
    ...
    do {
      ...
      if (lt_clock(wrt_clk, local_clk)) { /* TIME: < */
        read_set_add(&self->isolation, p_act_vhdr, p_latest, (void *)p_copy);
        return (void *)p_copy;
      }
      ...
      if (unlikely(
              lt_clock(chs->cpy_hdr.wrt_clk_next, last_ckpt_clk))) /* TIME: < */
        break;
...
static int try_alloc_act_vhdr(void *obj) {
  ...
  if (p_act_nvhdr->p_act_vhdr ||
      !smp_cas(&p_act_nvhdr->p_act_vhdr, NULL, new_act_nvhdr.p_act_vhdr)) {
...
int _ts_try_lock(ts_thread_struct_t *self, void **pp_obj, size_t size) {
  ...
  if (unlikely(p_lock)) {
    ...
    ts_assert(ws);
    if (self != ws->thread || self->run_cnt != ws->thread->run_cnt) {
      return 0;
    }
  ...
  if (p_old_copy) {
    ...
    local_clk = self->local_clk;
    if (gte_clock(get_wrt_clk(chs, local_clk), local_clk)) /* TIME: >= */
      return 0;
  }
  /* this check will catch if copy is free-ed but still the
  * live ptr exists i.e., before checkpointing tombstone and
  * physically free-ing the master
  if (is_ptr_valid(p_act_vhdr, self->local_clk) == ETEMPORAL) {
                  perror("Temporal safety (user-after-free) violation detected
  (cpy_obj)\n");
                  // FIXME: return ETEMP
                  exit(0);
          }*/
  ...
  chs = tvlog_append_begin(&self->tvlog, p_act_vhdr, size, &bogus_allocated);
  ...
  *pp_obj = (void *)p_new_copy;
  ...
  ts_assert(p_act_vhdr ==
            obj_to_ahs(get_org_act_obj(obj))->act_nvhdr.p_act_vhdr);
  ts_assert(obj_to_ahs(get_org_act_obj(obj))->act_nvhdr.p_act_vhdr->p_lock !=
            NULL);
  return 1;
}
