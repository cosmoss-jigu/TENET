...
static inline int port_tvlog_region_init(unsigned long size,
                                         unsigned long num) {
...
static inline void *port_alloc_tvlog_mem(void) {
  ...
  retry:
    ...
    for (j = 0; j < 64; ++j) {
      ...
      if ((bitmap & mask) == 0) {
        if (!smp_cas(&g_lr.bitmap[i], bitmap, bitmap | mask))
          goto retry;
        /* Succeed */
...
static inline void port_free(void *ptr) { free(ptr); }
...
static void port_finish_thread(void *x) { /* do nothing */
}
