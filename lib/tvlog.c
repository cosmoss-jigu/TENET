...
static inline unsigned int add_extra_padding(ts_tvlog_t *tvlog,
  ...
  if (bogus == 0 && tvlog_index(tvlog->tail_cnt + tvlog_size + extra_size) <
                        tvlog_index(tvlog->tail_cnt)) {
    padding = TS_TVLOG_SIZE - tvlog_index(tvlog->tail_cnt + tvlog_size);
  }
  if (padding) {
    ts_assert(tvlog_index(tvlog->tail_cnt + tvlog_size + padding) == 0);
  }
...
void tss_insert_canary_cpy(ts_cpy_hdr_struct_t *chs) {
  uint64_t *p_obj_canary = (uint64_t *)obj_canary_from_chs(chs);
  uint64_t ohdr_canary_mask = (uint64_t)&chs->obj_hdr.canary;
  uint64_t obj_canary_mask = (uint64_t)p_obj_canary;

  chs->cpy_hdr.canary = get_canary_64();
  chs->obj_hdr.canary = chs->cpy_hdr.canary ^ ohdr_canary_mask;
  *p_obj_canary = chs->obj_hdr.canary ^ obj_canary_mask;
}
...
ts_cpy_hdr_struct_t *tvlog_append_begin(ts_tvlog_t *tvlog,
  ...
  *bogus = 0;
  ...
  tss_insert_canary_cpy(chs);

#ifdef TS_ENABLE_STATS
  if (chs->obj_hdr.type == TYPE_COPY) {
    stat_thread_acc(tvlog_to_thread(tvlog), n_tvlog_cpobj_written_bytes,
                    get_entry_size(chs));
  }
#endif
  return chs;
...
#define ws_for_each_safe(tvlog, ws, obj_idx, log_cnt, log_cnt_next)            \
  for ((obj_idx) = 0, (log_cnt) = ws_iter_begin(ws),                           \
      (log_cnt_next) = ws_iter_next(log_cnt, tvlog_at_chs(tvlog, log_cnt));    \
       (obj_idx) < (ws)->num_objs; ++(obj_idx), (log_cnt) = (log_cnt_next),    \
      (log_cnt_next) = ws_iter_next(log_cnt, tvlog_at_chs(tvlog, log_cnt)))
...
static int validate_copy_obj(ts_cpy_hdr_struct_t *chs) {
  uint64_t *p_obj_canary = (uint64_t *)obj_canary_from_chs(chs);
  uint64_t c0 = chs->cpy_hdr.canary;
  uint64_t c1 = chs->obj_hdr.canary;
  uint64_t c2 = *p_obj_canary;
  uint64_t c1_mask = (uint64_t)&chs->obj_hdr.canary;
  uint64_t c2_mask = (uint64_t)p_obj_canary;

  if (c1 == (c0 ^ c1_mask) && c2 == (c1 ^ c2_mask)) {
    ts_trace(TS_DEBUG, "=======================================\n");
    ts_trace(TS_DEBUG, "### copy object validated ### \n");
    ts_trace(TS_DEBUG, "C0= %ld, C1= %ld, C2= %ld \n", c0, c1, c2);
    ts_trace(TS_DEBUG, "=======================================\n");
    return 1;
  }
  // ts_assert(0 && "spatial safety violation for copy object");
  return 0;
}

static int ws_move_lock_to_copy(ts_tvlog_t *tvlog, ts_ptr_set_t *free_set) {
  ts_wrt_set_t *ws;
  ts_cpy_hdr_struct_t *chs;
  void *p_act;
  unsigned long cnt;
  ...
  ws_for_each(tvlog, ws, i, cnt) {
    ...
    /* validate cpy obj before moving it to
     * the version chain*/
    if (!validate_copy_obj(chs)) {
      ts_trace(TS_INFO,
               "Spatial Safety Violation Detected in Tx Commit (copy obj)\n");
      return 0;
    }
    ...
    if (unlikely(num_free > 0 && ptrset_is_member(free_set, p_act))) {
    ...
    while (1) {
      ...
      if (p_old_copy == NULL) {
        ...
        wrt_clk_next = get_raw_wrt_clk(vobj_to_chs(p_old_copy, TYPE_COPY));
      ...
      if (smp_cas_v(&p_act_vhdr->p_copy, p_old_copy, chs->obj_hdr.obj,
                    p_old_copy2))
        break;
...
static void ws_unlock_commit(ts_tvlog_t *tvlog) {
  ...
  ws_for_each(tvlog, ws, i, cnt) {
    ...
    case TYPE_COPY:
      ...
      ts_assert(p_act_vhdr->p_lock == chs->obj_hdr.obj);
      if (!smp_cas(&p_act_vhdr->p_lock, chs->obj_hdr.obj, NULL)) {
    ...
    case TYPE_FREE:
      ...
      smp_atomic_store(&chs->cpy_hdr.__wrt_clk, wrt_clk);
      ts_assert(p_act_vhdr->ptr_tag != INVALID_PTR_TAG);
      smp_atomic_store(&p_act_vhdr->ptr_tag, INVALID_PTR_TAG);
      smp_mb();
      smp_wmb_tso();
...
int tvlog_commit(ts_tvlog_t *tvlog, ts_oplog_t *oplog,
                 ts_oplog_t *oplog_replica, ts_ptr_set_t *free_set,
  ...
  if (!ws_move_lock_to_copy(tvlog, free_set))
    return 0;

  /* Get the write clock */
  smp_atomic_store(&tvlog->cur_wrt_set->pending_wrt_clk, new_clock(local_clk));
  advance_clock();
  ...
  /* Persist and commit the oplog replica*/
  oplog_enq(oplog_replica, local_clk, pending_wrt_clk, op_info);
  oplog_enq_persist(oplog_replica);
#endif
  ...
  if (likely(!oplog_try_request_ckpt_if_needed(oplog, oplog_replica))) {
...
static void _dbg_poision_wrt_set(ts_tvlog_t *tvlog, unsigned long start_cnt) {
  ...
  ws_for_each_safe(tvlog, ws, i, cnt, cnt_next) {
    chs = tvlog_at_chs(tvlog, cnt);
    memset((void *)chs->obj_hdr.obj, TS_FREE_POSION, chs->obj_hdr.obj_size);
...
static int tvlog_reclaim_x(ts_tvlog_t *tvlog, ts_ckptlog_t *ckptlog,
                           ts_ckptlog_t *ckptlog_replica, int mode) {
  ts_wrt_set_t *ws;
  ts_wrt_set_struct_t *wss;
  ts_cpy_hdr_struct_t *chs = NULL;
  ts_act_vhdr_t *p_act_vhdr;
    ...
    ws_for_each(tvlog, ws, i, cnt) {
      ...
      case TYPE_COPY:
        ...
        if (mode == RECLAIM_TVLOG_BEST_EFFORT &&
            gte_clock(chs->cpy_hdr.wrt_clk_prev, ckpt_s_clk)) { /* TIME: >= */
        ...
        _dbg_assert_is_latest_copy(chs, mode, last_ckpt_clk, ckpt_s_clk,
                                   TYPE_COPY);
        /* Reclaim this copy */
        break;
      case TYPE_BOGUS:
      ...
      case TYPE_FREE:
         ...
         * so just skip. */
        if (unlikely(chs->obj_hdr.status == STATUS_TOMBSTONE_MARKED))
          break;
        ...
        if (p_act_vhdr->np_cur_act == p_act_vhdr->np_org_act) {
          /* Replace __wrt_clk to tombstone_clk,
           * which is ckpt_s_clk. */
          p_act_vhdr->tombstone_clk = ckpt_s_clk;
          smp_wmb_tso();
          /* ... then append its tombstone. */
          ckptlog_enq_tombstone(ckptlog, ws->wrt_clk, ckpt_s_clk, chs, false);
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
          /* CKPT to NUMA replica*/
          ckptlog_enq_tombstone(ckptlog_replica, ws->wrt_clk, ckpt_s_clk, chs,
                                true);
#endif
          is_ckptlog_dirty = 1;
         ...
         * the current checkpointed master. */
          ...
          ckpt_entry = vobj_to_ckpt_ent(p_act_vhdr->np_cur_act);
          np_tombstone = &ckpt_entry->ckptlog_hdr.tombstone;
          ts_assert(*np_tombstone == 0);
          smp_atomic_store(np_tombstone, TOMBSTONE);
          clwb(np_tombstone);
          ts_trace(TS_NVMFREE, "[%s:%d] MARK_TOMBSTONE (ORG_ACT): %p\n",
                   __func__, __LINE__, p_act_vhdr->np_cur_act);
        }
...
stop_checkpoint_step1:
  ...
  while (start_cnt < tail_cnt) {
    ...
    if (gte_clock(wrt_clk, ckpt_s_clk) ||
      ...
      ts_trace(TS_TVLOG, "== TVLog Reclaim: ws->wrt_clk= %ld \n", wrt_clk);
      break;
    }
    ws_for_each(tvlog, ws, i, cnt) {
      chs = tvlog_at_chs(tvlog, cnt);
      ...
      if (chs->obj_hdr.status != STATUS_DETACHED) {
        ts_assert(chs->cpy_hdr.__wrt_clk != MAX_VERSION);
        _dbg_assert_is_at_borderline(__func__, __LINE__, chs, ckpt_s_clk,
                                     last_ckpt_clk);
        if (is_at_borderline(chs, ckpt_s_clk, last_ckpt_clk)) {
          if (mode == RECLAIM_TVLOG_CKPT) {
            /* validate the copy object
             * before checkpointing it*/
            if (!validate_copy_obj(chs)) {
              ts_trace(TS_INFO,
                       "Spatial Safety Violation detected for copy obj\n");
              return ESPATIAL;
            }
            ckptlog_enq(ckptlog, wrt_clk, ckpt_s_clk, chs, false);
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
            /*enq to the NUMA copy*/
            ckptlog_enq(ckptlog_replica, wrt_clk, ckpt_s_clk, chs, true);
#endif
            try_detach_obj(chs);
            ckpt_bytes += sizeof_chs(chs);
            /* TODO: modify 2*chs for WA measurement*/
            is_ckptlog_dirty = 1;
          } else {
            /* Revert start_cnt to the beginning
             * of the write set because we should
...
stop_checkpoint_step2:
...
#ifdef TS_ENABLE_CKPTLOG_REPLICATION
    /*persist the NUMA replica*/
    ckptlog_enq_persist(ckptlog_replica);
#endif
  }
  ...
  if (new_head_cnt != old_head_cnt) {
    ...
    stat_thread_acc(tvlog_to_thread(tvlog), n_tvlog_reclaimed_bytes,
                    reclaimed_bytes);
    ts_trace(TS_TVLOG, "%s [%p]: %ld bytes reclaimed (h=%ld, ph=%ld, t=%ld)\n",
...
void tvlog_reclaim(ts_tvlog_t *tvlog, ts_ckptlog_t *ckptlog,
  ...
  case RECLAIM_TVLOG_BEST_EFFORT:
    ret = tvlog_reclaim_x(tvlog, ckptlog, ckptlog_replica,
                          RECLAIM_TVLOG_BEST_EFFORT);
    if (likely(ret)) {
      stat_thread_inc(tvlog_to_thread(tvlog), n_tvlog_best_effort);
    }
    break;
  case RECLAIM_TVLOG_CKPT:
    ret = tvlog_reclaim_x(tvlog, ckptlog, ckptlog_replica, RECLAIM_TVLOG_CKPT);
    if (unlikely(ret == ESPATIAL)) {
      /* TODO: fix this return error then exiting
       * need to change in many places as tvlog return used in many palces*/
      perror("Spatial Safety Violation detected while Checkpointing the copy "
             "obj\n");
      exit(0);
    }
    if (likely(ret)) {
...
static int reclaim_tvlog_ckptlog(ts_tvlog_t *tvlog, ts_ckptlog_t *ckptlog,
  ...
  if (*tvlog->need_reclaim) {
    tvlog_reclaim(tvlog, ckptlog, ckptlog_replica);
    reclaim_cnt++;
  }
  if (*ckptlog->need_reclaim) {
    ckptlog_reclaim(ckptlog, ckptlog_replica);
    reclaim_cnt++;
  }
  return reclaim_cnt;
}
...
void tvlog_reclaim_below_high_watermark(ts_tvlog_t *tvlog,
  ...
  if (tvlog_used(tvlog) >= TS_TVLOG_HIGH_MARK) {
    ts_trace(TS_TVLOG, "tvlog: <<<<< blocking reclamation (%ld/%ld)\n",
    ...
    while (tvlog_used(tvlog) >= TS_TVLOG_HIGH_MARK) {
      ...
      do {
        ...
        reclaim_cnt = reclaim_tvlog_ckptlog(tvlog, ckptlog, ckptlog_replica);
      } while (!reclaim_cnt);
    }
...
int tvlog_create(ts_thread_struct_t *self, ts_tvlog_t *tvlog) {
  ...
  ts_assert(tvlog->buffer == align_ptr_to_cacheline((void *)tvlog->buffer));
