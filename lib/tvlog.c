		...
		ws_for_each (tvlog, ws, i, cnt) {
			...
			case TYPE_FREE:
				...
				if (p_act_vhdr->np_cur_act ==
					...
					ckptlog_enq_tombstone(ckptlog_replica,
							      ws->wrt_clk,
							      ckpt_s_clk, chs,
							      true);
#endif
		   ...
		   ts_ckptlog_t *ckptlog_replica)
{
	...
	case RECLAIM_TVLOG_CKPT:
		...
		if (unlikely(ret == ESPATIAL)) {
			...
			perror("Spatial Safety Violation detected while Checkpointing the copy obj\n");
			exit(0);
		}
		if (likely(ret)) {
	...
	if (tvlog_used(tvlog) >= TS_TVLOG_HIGH_MARK) {
		...
		while (tvlog_used(tvlog) >= TS_TVLOG_HIGH_MARK) {
			...
			do {
				...
				reclaim_cnt = reclaim_tvlog_ckptlog(
					tvlog, ckptlog, ckptlog_replica);
			} while (!reclaim_cnt);
		}
