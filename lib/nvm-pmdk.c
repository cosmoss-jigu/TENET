...
ts_nvm_root_obj_t *load_master_obj_numa_pool(const char *path, size_t sz)
{
	...
	if (access(POOL_PATH_MASTER_NUMA, F_OK) != 0) {
		__g_master_pool_numa =
			pmemobj_create(POOL_PATH_MASTER_NUMA,
				       POBJ_LAYOUT_NAME(master - numa),
				       POOL_SIZE_MASTER, 0666);
		if (unlikely(!__g_master_pool_numa)) {
			ts_trace(TS_ERROR,
				 "failed to create master obj numa pool\n");
			return NULL;
		}
	} else {
		if ((__g_master_pool_numa = pmemobj_open(
			     POOL_PATH_MASTER_NUMA,
			     POBJ_LAYOUT_NAME(master - numa))) == NULL) {
			ts_trace(
				TS_ERROR,
				"failed to open the existing master numa pool\n");
			return NULL;
		}
	}
...
ts_nvm_root_obj_t *load_master_obj_pool(const char *path, size_t sz)
{
	...
	if (access(POOL_PATH_MASTER, F_OK) != 0) {
		__g_master_pool =
			pmemobj_create(POOL_PATH_MASTER,
				       POBJ_LAYOUT_NAME(master - local),
				       POOL_SIZE_MASTER, 0666);
		if (unlikely(!__g_master_pool)) {
			ts_trace(TS_ERROR,
				 "failed to create master obj local pool\n");
			return NULL;
		}
	} else {
		if ((__g_master_pool = pmemobj_open(
			     POOL_PATH_MASTER,
			     POBJ_LAYOUT_NAME(master - local))) == NULL) {
			ts_trace(
				TS_ERROR,
				"failed to open the existing master local pool\n");
			return NULL;
		}
	}
				      ...
				      int *is_created)
{
	...
	if (access(POOL_PATH_NUMA, F_OK) != 0) {
		__g_pop_numa = pmemobj_create(POOL_PATH_NUMA,
					      POBJ_LAYOUT_NAME(replica),
					      POOL_SIZE, 0666);
		...
		*is_created = 1;
		...
		if ((__g_pop_numa = pmemobj_open(POOL_PATH_NUMA,
						 POBJ_LAYOUT_NAME(replica))) ==
		    NULL) {
			ts_trace(TS_ERROR,
				 "failed to open the existing NUMA pool\n");
			return NULL;
		}
		*is_created = 0;
	}
