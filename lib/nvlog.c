...
static ts_nvlog_store_t *_nvlog_store_create(unsigned long size,
  ...
  if (unlikely(!is_power_of_2(size))) {
    ts_trace(TS_ERROR, "The requested log size is not power of 2\n");
    goto err_out;
  }
  pthread_mutex_lock(&nvlog_lock);
  ...
  if (!is_replica) {
    ...
    //	nvlog_store = (ts_nvlog_store_t *)nvm_alloc_numa(
    //		sizeof(*nvlog_store));
    nvlog_store = (ts_nvlog_store_t *)nvm_alloc(sizeof(*nvlog_store));
    if (!nvlog_store) {
      ts_trace(TS_ERROR, "failed to allocate nvlog_store\n");
  ...
  /********************************************************************************
   * alloc (size + CACHE_LINE_SIZE), after the allocation the nvlog * in nvm
   *pool looks like							*
   *										*
   *										*
   *+----------+
  ...
  if (!is_replica)
    ...
    // nvlog_store->_buffer = (unsigned char *)nvm_alloc_numa(
    //	size + TS_CACHE_LINE_SIZE);
    nvlog_store->_buffer =
        (unsigned char *)nvm_alloc(size + TS_CACHE_LINE_SIZE);
    ts_trace(TS_DEBUG, "nvlog_store_numa: %p\n", nvlog_store);
  }
  ...
  nvlog_store->_buffer = (unsigned char *)malloc(size + TS_CACHE_LINE_SIZE);
#endif /* TS_NVM_IS_DRAM */

  if (unlikely(!nvlog_store->_buffer)) {
    goto err_out;
  }
  nvlog_store->buffer = align_ptr_to_cacheline((void *)nvlog_store->_buffer);
  /* NOTE:
...
int nvlog_create(ts_thread_struct_t *self, ts_nvlog_t *nvlog,
  ...
  nvlog_store = _nvlog_store_create(size, type, status, is_replica);
  if (unlikely(!nvlog_store)) {
    return ENOMEM;
  }
...
ts_nvlog_entry_hdr_t *nvlog_enq(ts_nvlog_t *nvlog, unsigned int obj_size) {
  ...
  if (unlikely(nvlog_index(nvlog, nvlog->tail_cnt + entry_size) <
    ...
    bogus_size = nvlog->log_size - nvlog_index(nvlog, nvlog->tail_cnt);
...
void nvlog_enq_persist(ts_nvlog_t *nvlog) {
   ...
   */
    ...
    for (i = nv_tail_cnt; i < nvlog->log_size; i += TS_CACHE_LINE_SIZE) {
      clwb(buffer + i);
    }
