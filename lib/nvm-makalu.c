...
static void map_persistent_region(const char *path, size_t sz,
                                  int *is_created) {
  void *addr;
  int fd, result;
  ...
  *is_created = 1;
  ...
  addr = mmap(0, sz, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0);
...
void nvm_heap_destroy(void) { MAK_close(); }

void *nvm_alloc(size_t size) { return MAK_malloc(size); }
...
void nvm_free(void *ptr) { MAK_free(ptr); }
